...
public class PhotoLibrary extends CordovaPlugin {
    ...
    public CordovaResourceApi.OpenForReadResult handleOpenForRead(Uri uri) throws IOException {
        ...
        boolean isPhotoThumbnail = origUri.getHost().toLowerCase().equals("thumbnail") && origUri.getPath().isEmpty();
        boolean isPhoto = origUri.getHost().toLowerCase().equals("photo") && origUri.getPath().isEmpty();
        boolean isVideo = origUri.getHost().toLowerCase().equals("video") && origUri.getPath().isEmpty();

        if (!isPhotoThumbnail && !isPhoto && !isVideo) {
            throw new FileNotFoundException("URI not supported by PhotoLibrary");
        }

        String resId = origUri.getQueryParameter("photoId");

        if (resId == null || resId.isEmpty()) {
            resId = origUri.getQueryParameter("videoId");
        }

        if (resId == null || resId.isEmpty()) {
            throw new FileNotFoundException("Missing 'photoId' or 'videoId' query parameter");
        }

        if (isPhotoThumbnail || isVideo) {
            ...
            if (isPhotoThumbnail) {
                ...
                PhotoLibraryService.PictureData thumbnailData = service.getThumbnail(getContext(), resId, width, height, quality);
                ...
                //Output Video First Frame Thumbnail

                PhotoLibraryService.PictureAsStream pictureAsStream = service.getVideoThumbnail(resId, width, height);
        ...
        } else if (isPhoto) { // isPhoto == true

            PhotoLibraryService.PictureAsStream pictureAsStream = service.getPhotoAsStream(getContext(), resId);
            InputStream is = pictureAsStream.getStream();
